...
import { changestatus, compettior, leaddropdown, enquriy } from '../../../../axios/Services';
...
function UpdateStatusModal({ updateStatus, onclose, updatstatuserid }) {
  ...
  return (
    <Modal title="Update Status" open={updateStatus} onCancel={onclose} onOk={formik.handleSubmit}>
      <Form layout="vertical">
        <Form.Item label="Status">
          ...
          {formik.touched.leadStatus && formik.errors.leadStatus ? <div>{formik.errors.leadStatus}</div> : null}
        </Form.Item>
            ...
            <Form.Item label="Competitor">
              ...
              >
                {dropdownCompetitor.map((value) => (
                  ...
                  <Option key={value.competitorId} value={value.competitorId} values={value.competitorName}>
                    {value.competitorName}
                  </Option>
                ))}
              </Select>
              {formik.touched.competitor && formik.errors.competitor ? <div>{formik.errors.competitor}</div> : null}
            </Form.Item>
            <Form.Item label="comment">
              <TextArea name="comment" value={formik.values.comment} onChange={formik.handleChange} />
        ...
        {(formik.values.leadStatus === 3 ) && (
          <>
            ...
            <Form.Item label="demo_date">
              <DatePicker  name="demo_date" showTime onChange={(date,datestring) => formik.setFieldValue('demo_date',datestring)} />
              {formik.touched.date && formik.errors.date ? <div>{formik.errors.date}</div> : null}
            </Form.Item>
            <Form.Item label="comment">
              <TextArea name="comment" value={formik.values.comment} onChange={formik.handleChange} />
              {formik.touched.comment && formik.error.comment ? <div>{formik.errors.comment}</div> : null}
            </Form.Item>
          </>
        )}
...
{(formik.values.leadStatus === 24 ) && (
          <>
            ...
            <Form.Item label="poc_date">
              <DatePicker  name="poc_date" showTime onChange={(date,datestring) => formik.setFieldValue('poc_date',datestring)} />
              {formik.touched.poc_date && formik.errors.poc_date ? <div>{formik.errors.poc_date}</div> : null}
            </Form.Item>
            <Form.Item label="comment">
              <TextArea name="comment" value={formik.values.comment} onChange={formik.handleChange} />
              {formik.touched.comment && formik.errors.comment ? <div>{formik.errors.comment}</div> : null}
        ...
        {formik.values.leadStatus === 5 && (
          <>
            <Form.Item label="follow_up_date">
              <DatePicker onChange={(date,datestring) => formik.setFieldValue('follow_up_date',datestring)} />
              {formik.follow_up_date && formik.follow_up_date ? <div>{formik.follow_up_date}</div> : null}
            </Form.Item>
            <Form.Item label="enquiry_type">
              <Select
                name="enquiry_type"
                value={formik.values.enquiry_type}
                onChange={(value) => formik.setFieldValue('enquiry_type', value)}
              >
                {enquirydrop.map((ele) => (
              ...
              {formik.touched.enquiry && formik.errors.enquiry ? <div>{formik.errors.enquiry}</div> : null}
            </Form.Item>
          </>
        )}

        {(formik.values.leadStatus === 17 || formik.values.leadStatus === 6 || formik.values.leadStatus === 2) && (
          <Form.Item label="comment">
            <TextArea name="comment" value={formik.values.comment} onChange={formik.handleChange} />
            {formik.touched.comment && formik.errors.comment ? <div>{formik.errors.comment}</div> : null}
          </Form.Item>
